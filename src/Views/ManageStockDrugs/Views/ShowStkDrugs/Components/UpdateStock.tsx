...
const UpdateStockView =(props:IProps)=>{
    ...
    const pharmasClasses=(store.getState().user.userIdentity.user as IStockUser).pharmasClasses;
    ...
    const handleForClassChange:ISelectInputChange=(e,child)=>{
        setStkProdData(prev=>({...prev,forClassId:e.target.value as string}))
    }
    const handleSubmit=()=>{
        ...
        formData.append("forClassId",stkProdData.forClassId);
            ...
            <Dialog className={classes.dialog} onClose={handleModalClose} aria-labelledby="customized-dialog-title" open={openModal}>               
                ...
                <DialogContent dividers>
                    ...
                    <Box>
                        ...
                        <Box>
                            <FormControl  className={stkProdError.ForClassId?classes.formControl_withError:classes.formControl}>
                                <InputLabel id="reportSelect" className={(classes as any).inputLabel}>
                                                اختر تصنيف هذا التقرير
                                </InputLabel>       
                                <Select
                                    variant="outlined"
                                    id="ForClassSelect"   
                                    name="ForClassId"                                         
                                    value={stkProdData.forClassId}
                                    onChange={handleForClassChange}
                                    >
                                    {pharmasClasses.map((item,i)=>(
                                    <MenuItem key={i} value={item.id}>{item.name}</MenuItem>
                                    ))}
                                </Select>
                                    {<FormHelperText className={classes.customeErros}>{displayError(stkProdError.ForClassId)}</FormHelperText>}
                            </FormControl>
                        </Box>
                        <Box my={1}>
